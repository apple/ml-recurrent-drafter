...
def benchmark_customized_attention_mask(N: int, query_len: int = 0):
    def func() -> mx.array:
        ...
        mask = recurrent_drafting.mlx.attention.causal_mask(padding_mask, query_len)
        bias = recurrent_drafting.mlx.attention.bias(mask, dtype=mx.int32)
        return bias
    ...
    toc = time.perf_counter()
    tpi = 1e3 * (toc - toi) / 100
    print(f"recurrent_drafting.mlx.attention.causal_mask takes {tpi:.3f} (ms)")
